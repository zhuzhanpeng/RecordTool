 ...
 */
package com.onairm.baselibrary.recycleview;
...
import com.onairm.baselibrary.recycleview.animation.AlphaInAnimation;
import com.onairm.baselibrary.recycleview.animation.BaseAnimation;
import com.onairm.baselibrary.recycleview.animation.ScaleInAnimation;
import com.onairm.baselibrary.recycleview.animation.SlideInBottomAnimation;
import com.onairm.baselibrary.recycleview.animation.SlideInLeftAnimation;
import com.onairm.baselibrary.recycleview.animation.SlideInRightAnimation;
import com.onairm.baselibrary.recycleview.entity.IExpandable;
import com.onairm.baselibrary.recycleview.loadmore.LoadMoreView;
import com.onairm.baselibrary.recycleview.loadmore.SimpleLoadMoreView;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void updateDatSet(List<T> tempList, int type) {

        if (null == tempList || tempList.size() == 0)
            return;

        int startPos = mData.size();
        if (mData.size() == 0) {//第一次初始化直接返回获取来的数据
            setNewData(tempList);
        } else if (type == 1) {//下拉刷新
            setNewData(tempList);
        } else {//加载分页
            ...
            notifyItemRangeInserted(startPos + getHeaderLayoutCount(), mData.size() - startPos);
        }
    }

    public void handleNewData(List<T> newData) {
        if(page==0){
            updateDatSet(newData, 1);
        }else{
            updateDatSet(newData, 2);
        }
    }
    /**
     * @param responsePageNum server返回的pageSize
     *
    * */
    public void handlePageNum(int responsePageNum) {
        if(responsePageNum>=requestPageSize){
            ++this.page;
        }
    }
