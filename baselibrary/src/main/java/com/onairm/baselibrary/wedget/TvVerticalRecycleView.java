...
public class TvVerticalRecycleView extends RecyclerView {

    private static final String TAG = TvVerticalRecycleView.class.getSimpleName();
...
//    private ItemListener mItemListener;
    ...
    public TvVerticalRecycleView(Context context) {
        super(context);
        init(context);
    }

    public TvVerticalRecycleView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public TvVerticalRecycleView(Context context, @Nullable AttributeSet attrs, int defStyle) {
...
//                    mOnItemListener.onItemClick(TvVerticalRecycleView.this,v,getChildLayoutPosition(v));
//                }
//            }
//            /**
//             * 子控件焦点改变事件
...
//                Log.d("TVRecyclerView"," item position :"+getChildLayoutPosition(v) +" ,,,, hasFocus :"+hasFocus);
//                if (mOnItemListener!=null){
//                    mOnItemListener.onItemSelected(TvVerticalRecycleView.this,v,getChildAdapterPosition(v));
//                }
//            }
//
//        };
...
//        Log.i(TAG, "last-->left:" + lastChild.getLeft() + "   right:" + lastChild.getRight());
        ...
        int centerBottom = 0;
        if (targetChild != null) {
            childTopPx = targetChild.getTop();//子view相对于父view的左边距
            childBottomPx = targetChild.getBottom();//子view相对于父view的右边距
            childHeight = targetChild.getHeight();
            centerTop = parentHeight / 2 - childHeight / 2;//计算子view居中后相对于父view的左边距
            centerBottom = parentHeight / 2 + childHeight / 2;//计算子view居中后相对于父view的右边距
        }
...
//        Log.i(TAG, "rv width:" + parentWidth + "   item width:" + childWidth + "   centerleft:" + centerLeft + "   centerRight:" + centerRight);
        if (childTopPx > centerTop) {//子view左边距比居中view大（说明子view靠父view的右边，此时需要把子view向左平移
            ...
            mLasty = childTopPx;
            if ((position / 2) % 2 ==0 && position != 0) {
                int aa  = centerTop - childTopPx + 40;
                mScroller.startScroll(0, childTopPx, 0, aa, 600);//600为移动时长，可自行设定
            } else {
...
//        void onItemClick(TvVerticalRecycleView parent, View itemView, int position);
//        void onItemSelected(TvVerticalRecycleView parent, View itemView, int position);
//    }
//
//
