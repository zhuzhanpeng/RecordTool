 ...
 */
public abstract class BaseApi implements Func1<BaseData, BaseData> {
    //rx生命周期管理
    ...
    /*有网情况下的本地缓存时间默认60秒*/
    private int cookieNetWorkTime = 0;
    ...
    @Override
    public BaseData call(BaseData httpResult) {
        if (httpResult.getStatusCode() != 0) {
            throw new HttpTimeException(httpResult.getStatusCode());
        }
